...
plot_shadows <- function(wrapper_object, pooled = T, ...) {
  max_step <- wrapper_object %>% length()
  ...
  ggplot(vimps, aes(y=reorder(varname, VIMP, FUN = median), x = VIMP, fill= decision), ...) + geom_boxplot(outlier.size = 0.5)  + 
    #scale_x_discrete(labels = function(labels) str_replace_all(labels, "_", " ")) +
    #stat_summary(fun = function(y) quantile(y, 0.995), geom = "point", color = "red", size = 3) + 
    theme_bw() + theme(...)
}

plot_vimps(alz_three_steps, axis.title=element_blank(), legend.position = "none", text=element_text(size=20))
plot_shadows(alz_three_steps, axis.title=element_blank(), legend.position = "none", text=element_text(size=20))
