...
dir_res <- file.path(here::here(), "results_supplementary", "intermediate_results")
...
perf_proposed %>% 
  writexl::write_xlsx(path = file.path(dir_res, "perf_proposed_all_supplementary.xlsx"))


# Code generating TABLE 2
dir_tables <-  file.path(here::here(), "results_supplementary", "tables")
if(!dir.exists(dir_tables)){
  dir.create(dir_tables)
}

data.table::rbindlist(list(perf_proposed %>% filter(Method == 'Proposed with preselect' 
                                                    & type == 'pooled' 
                                                    & iterprop == '1',
  ...
  arrange(design) %>%
  writexl::write_xlsx(path = file.path(dir_tables, "table2_supplementary.xlsx"))
...
data.table::rbindlist(list(select_proposed_nico), fill = T ) %>% 
  ...
  writexl::write_xlsx(path = file.path(dir_tables, "table3_supplementary.xlsx"))


dir_logs <-  file.path(here::here(), "results_supplementary", "logs")
if(!dir.exists(dir_logs)){
  dir.create(dir_logs)
}
writeLines(capture.output(sessionInfo()), file.path(dir_logs, "sessionInfo_02_unpack_results_supplementary.txt"))
